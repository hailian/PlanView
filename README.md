...
- **协议配置 ProtocolConfig**：拆帧方式（TLV / 帧头+Length 及参数）+ 规约字段（属性面板「规约字段」区增删编辑：字段名 / TLV 槽位 / 偏移 / 类型 / 标签地址）；一个协议可被多个数据源复用
- **数据源 DataSource**：传输（UDP 绑定本地端口 / TCP 连接远端）+「关联协议」下拉（动态列出工程内全部协议配置组件名）

运行器取首个数据源组件 + 其关联的协议配置合成接入（`frameSettingsFromProject`）：解析值写入对应槽位标签（标签 scale/offset 照常生效），顶栏「报文监视」实时展示原始帧 HEX 与解析结果。协议缺失时按默认 TLV 无字段收帧；无数据源组件回退旧工程设置或 SoftG 行协议。v1 只收不发。校验面板检查：数据源未关联/关联不存在、协议未被引用、协议重名、多数据源。

配套模拟器 `python tools/udp_frame_sim.py 9001` 与示例工程 `examples/demo_frame_project.json`（数据源 + 泵站TLV协议 两组件）。
