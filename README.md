...
## 帧数据源（TCP/UDP + 自配置规约）组件

数据源是「通信」分类的**画布组件**：从组件面板拖到页面即启用，选中后在属性面板配置——

- **传输**：UDP（绑定本地端口）或 TCP 客户端（远端主机/端口）
- **拆帧**（TCP 字节流）：TLV（T/L 字节数、字节序、L 是否含帧头）或 固定帧头+Length；UDP 数据报天然成帧
- **规约字段**（属性面板「规约字段」区，可增删）：字段名 / TLV 槽位 / 偏移（TLV 相对负载 V，帧头模式相对整帧）/ 类型（u8..f64）/ 字节序 / 标签地址；TLV 模式按槽位匹配帧，解析值写入对应槽位标签（标签 scale/offset 照常生效）
- 组件在页面任意位置（含多页工程）；多个数据源组件时仅第一个生效（校验面板告警）。v1 只收不发
- 运行器顶栏「报文监视」实时展示原始帧 HEX 与按规约解析结果
- 配套模拟器 `python tools/udp_frame_sim.py 9001` 与示例工程 `examples/demo_frame_project.json`
