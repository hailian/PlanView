...
- **数据源 DataSource** —— 定义"从哪收"：
  ...
  - **自发（本地模拟设备）**：第五种传输方式——**只需配置发送周期（20ms~60s）**，无地址/端口/网卡任何网络项；按关联协议的格式周期性产帧**直接驱动自身解析管线**（字段值按类型确定性递增：整数 0..类型最大值环回、枚举按映射表遍历、字符串每字节 `a..z` 递增、布尔 0/1 翻转、浮点 0..100 步进），标签、报文监视、数据目的转发全部照常工作——零外部依赖即可演示/联调整条组态链路
...
- **数据目的 DataSink** —— 按名称关联数据源，运行器把收到的**原始帧原样转发**到 TCP/UDP/串口端点（网关/上传场景）；传输与数据源同构：TCP 客户端/服务端（**服务端=随启动即监听**，平台可在数据到来前接入，向接入方转发，接入前的帧按尽力而为丢弃）、UDP 客户端/服务端、**UDP 组播**（角色选「组播」：host=组地址、远端端口=组端口，发往组无需加入组，多方订阅同收）、串口；尽力而为转发（断线丢帧、2s 退避重连），随数据源启停同步
