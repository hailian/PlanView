...
- **数据目的 DataSink** —— 按名称关联数据源，运行器把收到的**原始帧原样转发**到 TCP/UDP/串口端点（网关/上传场景）；传输与数据源同构：TCP 客户端/服务端、UDP 客户端/服务端、**UDP 组播**（角色选「组播」：host=组地址、远端端口=组端口，发往组无需加入组，多方订阅同收）、串口；尽力而为转发（断线丢帧、2s 退避重连），随数据源启停同步
