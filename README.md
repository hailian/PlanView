...
| 设备以组播上报，多方需要同收一路数据 | UDP **组播**角色：绑定组端口并加入组播组（224~239 段组地址），端口复用允许同机多个消费者共收 |
...
- 打开工程（或命令行直接指定，或欢迎界面"打开上次"）即可投运；帧数据源「自动启动」默认关，顶栏手动启动/停止，勾选后打开即连；**纯监视/转发工程**（只有数据源/协议/数据目的、未绑定显示组件）同样启动数据链路——报文监视与数据目的转发不依赖标签绑定
...
- **协议配置 ProtocolConfig** —— 定义"怎么拆帧、字段在哪"：
  ...
  - 字段类型：`u8..f64` 整浮点、`bool`、定长 `string`、`enum` 枚举（取名称文本、未命中回退数值，映射行内弹窗编辑）、`hex`（长度可配，标签值为大写空格分隔的十六进制文本，如序列号/校验段展示）；数值字段支持 **scale 工程换算**（工程值 = 原始值 × scale）
  - 编辑体验：表格式行内编辑；新增字段 TLV 槽位默认继承前一条、偏移默认接续上一字段末尾
  - 「**生成字段组件**」按字段类型一键生成显示组件并自动绑定（bool→指示灯，其余→文本，纵向排列在协议组件下方）
- **数据源 DataSource** —— 定义"从哪收"：
  - TCP/UDP × 客户端/服务端四种角色 + **UDP 组播**（角色选「组播」：host=组地址（224~239 段）、localPort=组端口——绑定并加入组播组，同机多消费者可共收一组）+ **串口**（波特率 300..921600、数据位 5..8、校验无/奇/偶、停止位 1/2，字节流与 TCP 同一拆帧）
  ...
  - **USB** 传输（libusb/WinUSB 设备直连）：设备下拉经 libusb 实时枚举本机可打开的设备（`vid:pid[:序列号]` 作为工程持久化标识，同型号多台靠序列号区分），配置接口号与可选 IN/OUT 端点（默认自动选首个批量端点、无批量选中断）；字节流与串口同一拆帧。**libusb-1.0.dll 为可选运行时依赖**（放到程序目录或 System32，x64/x86 需与 exe 匹配；设备需 WinUSB/libusb 驱动，可用 [Zadig](https://zadig.akeo.ie) 安装），未放置仅该方式不可用并给提示，**构建/测试不依赖 libusb SDK**
  - **VISA** 传输（测试测量仪器统一接入）：经 NI-VISA / Keysight IO Libraries 等运行时以 `viRead`/`viWrite` 访问 **USBTMC（USB 仪器）/ GPIB / 以太网** 仪器——一个 VISA 资源地址（如 `TCPIP0::192.168.1.5::inst0::INSTR`、`GPIB0::5::INSTR`、`USB0::0x0957::0x1234::MY1::INSTR`）覆盖全总线，地址可手敲或经「枚举仪器」下拉（viFindRsrc）回填；字节流与串口同一拆帧，读超时 200ms。**VISA 运行时为可选运行时依赖**（visa64.dll/visa32.dll 动态加载，NI-VISA/Keysight/R&S 均装到 System32），未装仅该方式不可用并给安装提示，**构建/测试不依赖 VISA SDK**；与 USB 传输互补——裸批量端点设备走 USB，TMC/总线仪器走 VISA
  - **自发（本地模拟设备）**：第七种传输方式——**只需配置发送周期（20ms~60s）**，无地址/端口/网卡任何网络项；按关联协议的格式周期性产帧**直接驱动自身解析管线**（字段值按类型确定性递增：整数 0..类型最大值环回、枚举按映射表遍历、字符串每字节 `a..z` 递增、布尔 0/1 翻转、浮点 0..100 步进），标签、报文监视、数据目的转发全部照常工作——零外部依赖即可演示/联调整条组态链路
...
- **数据目的 DataSink** —— 按名称关联数据源，运行器把收到的**原始帧原样转发**到 TCP/UDP/串口/USB/VISA 端点（网关/上传场景）；传输与数据源同构：TCP 客户端/服务端（**服务端=随启动即监听**，平台可在数据到来前接入，向接入方转发，接入前的帧按尽力而为丢弃）、UDP 客户端/服务端、**UDP 组播**（角色选「组播」：host=组地址、远端端口=组端口，发往组无需加入组，多方订阅同收）、串口、**USB**（设备 OUT 端点发帧，配置与数据源同构）、**VISA**（viWrite 发往仪器，地址与数据源同构）；尽力而为转发（断线丢帧、2s 退避重连），随数据源启停同步
...
依赖（Dear ImGui `v1.92.9b-docking`、nlohmann/json）经 CMake FetchContent 自动拉取，默认 gitee 镜像、可用 `-D PV_IMGUI_REPO=...` 覆盖，首次配置需联网。Npcap 为**可选运行时依赖**（仅镜像抓包需要，见上）；libusb-1.0.dll 为**可选运行时依赖**（仅 USB 传输需要，x64/x86 需与 exe 匹配，见上）；VISA 运行时（NI-VISA / Keysight IO Libraries 等）为**可选运行时依赖**（仅 VISA 传输需要，见上）。产物直接位于 `out/build/<preset>/`：`LogicPlanner.exe`、`PageViewer.exe`、`pv_tests.exe`。
...
`test_Perf` 两个用例测量并打印吞吐参考值（x64-release 本机实测，2026-09-15 复测，随机器而异；Debug 构建自动 `[skip]`——未优化的数值没有参考意义）：
...
- **数据源→数据目的转发**：UDP 回环端到端——模拟设备发 2 万帧 → `FrameDataSource` 收帧并按规约解析 → 专用转发队列（与 200 条上限的报文监视日志**解耦**，高帧率不丢转发帧）经数据目的（UDP 客户端）**原样转发** → 平台侧接收端到账；全程零丢失对账（帧数/字节数/首帧字节一致、解析命中数齐全、标签值可读）——参考 **约 4 万帧/秒（1.4 MB/s）**。发送侧已做三项优化：连接态 socket 走 `send()` 快路径、内核收包缓冲加大到 1MB（允许更高突发节奏）、收包线程对端字符串缓存（同设备连发免逐包 `inet_ntop`）；剩余瓶颈是每帧两跳回环 syscall（UDP 数据报无法合并发送，Windows 无 `sendmmsg`），进一步提速需改运行器单线程 I/O 契约，不值当
- **组播接收→数据目的转发**：与单播转发同构的端到端基准（设备向组播组发 2 万帧 → 数据源加组收帧解析 → 数据目的单播转发 → 平台端到账，零丢失对账；先探测组播环回可达性，不可达自动 `[skip]`）——参考 **约 1.5~2 万帧/秒**，约为单播（4 万）的一半，瓶颈在组播发送路径的内核环回复制而非接收/解析
- **回环发包对比 sendto vs Npcap `pcap_sendpacket`**：验证"转发换驱动级注入是否提速"——在 Npcap 回环设备（DLT_NULL）上手工构 IPv4/UDP 头注入，可正常送达普通绑定的 UDP socket（送达率 100%），但吞吐**约低 11%**（6.1 万 vs 5.5 万帧/秒：NPF 驱动路径 vs Winsock 内核回环快路径）——**数据目的转发维持 socket 发送**；未装 Npcap 或无回环设备时该用例自动 `[skip]`
