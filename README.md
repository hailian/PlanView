...
- **数据源 DataSource**：**「自动启动」默认关**（PageViewer 打开工程不主动连接数据源，顶栏「启动数据源」手动启动/「停止数据源」停止；勾选后打开即连）+ 传输角色（TCP **客户端**连接远端端口 / TCP **服务端**监听本地端口等待接入 / UDP **服务端**监听本地端口 / UDP **客户端** connect 远端、本地端口由系统临时分配 / **串口** 打开 COM 口，波特率 300..921600、数据位 5..8、校验无/奇/偶、停止位 1/2；串口为字节流，与 TCP 走同一拆帧 / **监听** 固定三元组 dip/dport/协议 过滤，分两种监听方式：「**本机端口**」（默认，普通 socket——UDP 绑定 dport、命中=源为 dip:dport（反向）或 dip=`*` 全收（正向）；TCP 监听 dport、仅接受对端 IP=dip 的连接，未命中的报文/接入直接丢弃不驱动字段）与「**镜像抓包**」（Npcap 混杂模式——收**交换机镜像/SPAN 口上别人之间的流量**，需运行机器安装 [Npcap](https://npcap.com)；属性面板下拉选择抓包网卡，三元组命中=**任一方向**（正向=目的 dip:dport、反向=源 dip:dport，收完整会话），自动放行 802.1Q VLAN 标签帧；UDP 数据报天然成帧，TCP 按流（四元组+方向）逐流拆帧、尽力而为——镜像流的重传/乱序由拆帧器重同步兜底，不做序列号重组；未安装 Npcap 时连接报安装提示，其余功能不受影响，编译亦不依赖 Npcap SDK））+「关联协议/组」下拉（动态列出工程内全部协议配置与协议组，二选一）；关联后画布上以蓝色曲线连接两组件（最近边中点）
...
**可选运行时依赖 Npcap**（[www.npcap.com](https://npcap.com)）：数据源「监听-镜像抓包」方式（交换机 SPAN 场景）需要运行机器安装 Npcap 驱动；**构建与测试不需要**——代码经运行时 `LoadLibrary` 动态加载 `wpcap.dll`（自动探测 WinPcap 兼容模式与 `System32\Npcap` 安装位置），未安装时仅该监听方式不可用并给出提示，其余功能不受影响。
