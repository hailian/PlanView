...
- **回环发包对比 sendto vs Npcap `pcap_sendpacket`**：验证"转发换驱动级注入是否提速"——在 Npcap 回环设备（DLT_NULL）上手工构 IPv4/UDP 头注入，可正常送达普通绑定的 UDP socket（送达率 100%），但吞吐**约低 13%**（6.1 万 vs 5.4 万帧/秒：NPF 驱动路径 vs Winsock 内核回环快路径）——**数据目的转发维持 socket 发送**；未装 Npcap 或无回环设备时该用例自动 `[skip]`
