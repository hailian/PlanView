// FrameDataSource — 帧数据源组件：TCP/UDP/串口/USB/VISA 接入 + 自配置规约解析 + 标签槽位映射。
// TCP/串口/USB/VISA 字节流按 TLV / 帧头+Length 拆帧；UDP 数据报天然成帧。
// 传输角色：客户端 connect 远端 / 服务端监听（TCP listen / UDP bind）本地端口；串口/USB/VISA 无角色。
...
private:
    ...
    packet::VisaLink visa_;     // VISA 字节流（USBTMC/GPIB/以太网仪器）：与串口同路拆帧
